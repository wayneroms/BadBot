...
namespace FtxApi
{
    ...
    {
        public static string GetAuthRequest(FtxConfig client)
        {
            var time = FtxUtil.GetMillisecondsFromEpochStart();
        ...
        private static string GenerateSignature(FtxConfig config, long time)
        {
            var _hashMaker = new HMACSHA256(Encoding.UTF8.GetBytes(config.Secret));
